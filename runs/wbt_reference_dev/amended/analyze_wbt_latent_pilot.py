...
def load_arms(path: pathlib.Path | None) -> dict[str, dict[str, Any]]:
    """Load an arm specification, defaulting to the frozen Phase-0 arms.

    Each arm entry needs ``training_name``, ``actor_func``, ``critic_func``. An optional
    ``min_completion`` switches that arm's promotion rule from the baseline-relative frozen
    rule to an absolute completion floor (used by load-bearing arms like L1 that are not
    expected to beat the explicit-command baseline).
    """
    if path is None:
        return ARMS
    arms = json.loads(path.read_text())
    if not isinstance(arms, dict) or not arms:
        raise ValueError(f"{path} must contain a non-empty arm mapping")
    for arm, spec in arms.items():
        required = {"training_name", "actor_func", "critic_func"}
        if not isinstance(spec, dict) or not required.issubset(spec):
            raise ValueError(f"arm {arm!r} must define {sorted(required)}")
        if "min_completion" in spec and not isinstance(
            spec["min_completion"], (int, float)
        ):
            raise ValueError(f"arm {arm!r}: min_completion must be a number")
    return arms
...
def _validate_rollout_report(
    ...
    *,
    ...
    expected_seed: int = EVALUATION_SEED,
) -> list[str]:
    errors = []
    checks = {
        "schema_version": (report.get("schema_version"), 1),
        "passed": (report.get("passed"), True),
        "seed": (report.get("seed"), expected_seed),
        "training_name": (
...
def _promotion_decision(
    ...
    if min_completion is not None:
        # Absolute-floor rule for load-bearing arms (e.g. L1 latent-only command),
        # which are not expected to beat the explicit-command baseline.
        completion = metric_effects["completion"]["arm_mean"]
        passed = completion >= min_completion
        return {
            "rule": "absolute_completion_floor",
            "min_completion": min_completion,
            "completion": completion,
            "eligible_for_replication": passed,
        }
    ...
    for arm in arms:
        ...
        arm_summaries[arm] = {
            ...
            "promotion": _promotion_decision(
                metric_effects,
                min_completion=arms[arm].get("min_completion"),
            ),
        }
...
def main() -> None:
    ...
    parser.add_argument(
        "--arms-json",
        type=pathlib.Path,
        help="arm specification JSON; defaults to the frozen Phase-0 arms",
    )
