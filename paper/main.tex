...
\emph{window} retrieval hits 75--83\% top-1 at 1.02\% chance (98 windows); CKA between
per-robot encodings is 0.89--0.92. A \emph{linear} motion-category probe is near chance
(0.151 vs 0.125)---category structure, if present, is not linearly accessible (the same
caveat we apply to the embodiment probe below applies here; we therefore claim only
instance alignment, not semantic absence).
...
numbers: 7 held-out LAFAN1 clips, bootstrap CIs over per-clip means.
