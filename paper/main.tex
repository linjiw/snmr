...
explicit-command RL teacher, can be the actor's \emph{only} motion input at 0.965
ten-second completion versus the 0.98 teacher (1{,}024 rollouts; seed replication in
flight). Feeding the prior the frozen retargeting latent \emph{instead of} any
robot-space reference still reaches 0.606---62\% of teacher completion from a command
channel computed purely from human motion---where a goal-free prior collapses to 0.002.
...
\foreach \y/\lab/\v/\c in {0/{no goal}/0.002/corange, -6/{$z_{\mathrm{ret}}$ only (human-side)}/0.606/cgreen!70!black, -12/{ours (explicit goal)}/0.965/cgreen, -18/{explicit teacher}/0.98/cblue}{
...
\node[font=\footnotesize, text=cink2, anchor=west, align=left] at (-2mm,-26.5mm)
  {one recipe, 1{,}024 rollouts each;\\completion monotone in goal quality};
\end{scope}
\end{tikzpicture}
...
tracking policy's only motion command. (c)~Under one training regime (seed 0, 1{,}024
rollouts per bar), completion is monotone in what the prior sees: no goal collapses
(0.002), the frozen \emph{human-side} retargeting latent alone reaches 0.606, the
explicit robot-space goal reaches 0.965 against the 0.98 explicit-command teacher.}
\label{fig:teaser}
\end{figure*}
...
frozen latent carries 62\% of the teacher's completion (0.606 vs 0.98) where a goal-free
prior collapses to 0.002. The retargeting latent is not inert; it is
...
0.965 ten-second completion against its 0.98 teacher (Fig.~\ref{fig:teaser}c); replacing
the prior's robot-space goal with the frozen retargeting latent---so that \emph{no
robot-space reference exists anywhere in the student}---retains 0.606, and removing the
goal entirely collapses to 0.002. The same latent then absorbs a second, interaction-rich
...
\item \textbf{A learned command bottleneck for humanoid tracking.} A
  ...
  input---0.965 completion against a 0.98 explicit-command teacher with the reference
  never visible to the decoder; fed the frozen \emph{human-side} retargeting latent as
  its only goal source it retains 0.606 (goal-free control: 0.002)---plus a documented
...
\item \textbf{A falsification-first audit of a shared motion latent.} Pre-specified probe
  ...
  value: from ``inert'' to 62\% of teacher).
...
carries 0.606 completion through a co-trained prior (\S\ref{sec:exp}). The audit is the
...
actor's only motion input costs 1.5 points against its own teacher, where the design's
failed variants (posterior-path collection, goal-free priors) collapse to 0.001--0.002.
Second, the frozen retargeting latent alone---a channel computed purely from
\emph{human} motion, with no robot-space reference anywhere in the student---retains
0.606, thirty times the goal-free control; and the claim is causal, not correlational:
corrupting the latent at deployment collapses this arm to 0.000, so the completion flows
\emph{through} $z_{\mathrm{ret}}$, ruling out both gait-phase-from-proprioception (the
0.002 control) and prior memorization (the corruption control). The retargeting latent
transfers deployable goal information across the interface. Third, the remaining
36-point gap between
...
\begin{table}[t]
\centering
\caption{Command-interface factorial, one training regime (G1, walk1, seed 0,
1{,}024 rollouts; seed replication in flight).}
\label{tab:main}
\begin{tabular}{lcc}
\toprule
...
Explicit robot-space command & \textbf{0.965} & 0.126 \\
Explicit command $+$ frozen \snmr{} $z_{\mathrm{ret}}$ & 0.953 & 0.127 \\
Frozen \snmr{} $z_{\mathrm{ret}}$ only (human-side) & 0.606 & 0.163 \\
No goal (proprio-only prior) & 0.002 & 0.260 \\
\bottomrule
\end{tabular}
\end{table}
...
explicit goal is null ($0.953$ vs $0.965$; consistent with the shortcut mechanism and
with UniTracker's vanishing-influence observation). \emph{Substitutive} value is large:
$z_{\mathrm{ret}}$-only retains 0.606 where the goal-free control collapses to 0.002.
The right reading of the interface program is therefore substitution, not addition: the
latent matters where an explicit command is unavailable or unshareable---multi-clip and
cross-embodiment settings, where a 29-DoF target stream cannot command a 23-DoF robot at
all. Those settings remain in progress: multi-clip teachers at our compute budget have
not yet cleared their quality gate (0.47 mean completion at 1{,}024 envs $\times$ 16k
iters over 8 heterogeneous clips). The redundancy hypothesis---the human-side latent
should buy robustness when the robot-space reference is corrupted---now has its first
valid test (the leak-era version noised dimensions the decoder never read): on clean
checkpoints the beside-the-goal gap is directionally positive and grows with corruption
($+0.4/+1.2/+2.8$\,pp at $\sigma{=}0/0.25/0.5$; 1{,}024 rollouts per cell) but sits
below our pre-specified $+5$\,pp promotion gate at one training seed; the symmetric
control (corrupting the latent instead costs the combined arm 4\,pp) confirms the
policy reads both channels. We report the trend as directional, gated on the in-flight
seed replication.
...
reaches 0.606 with a \emph{frozen} encoder; the 36-point gap to the explicit-goal arm is
unclosed, and the specified lever (tracking gradients into the retargeting encoder) is
queued, as is seed replication of the factorial. The latent's additive value beside an
