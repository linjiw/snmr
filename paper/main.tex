...
noise floor. The variational machinery is not load-bearing here; what matters is
goal-conditioning, path consistency, and the exclusivity contract. The working design
compresses to one line: \emph{a goal-conditioned recoding, trained on the deployment
path, with the decoder blind to the goal}---the CVAE is one implementation of it, not
the mechanism.
