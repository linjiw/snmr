...
physically impoverished (contact F1 0.088 without supervision), and---frozen---inert as
a control input, a null that an independent ablation confirms. On the control side we
show a separate 64-d command bottleneck $z_{\mathrm{cmd}}$, produced by a
goal-conditioned residual-to-prior CVAE and DAgger-distilled from an explicit-command RL
teacher, can be the actor's \emph{only} motion input at $0.952\pm0.002$ ten-second
completion versus the $0.98$ teacher (three seeds), where naive alternatives fail
(frozen concatenation is harmful; goal-free priors reach 0.02--0.05). The retargeting
network also absorbs a second, interaction-rich teacher across three human skeletons,
cutting interaction-clip articulation error $3$--$5\times$ at $+1.05$\,cm on locomotion
(preliminary: ground-truth root). We release a pre-specified negative-results ledger and
a measurement-substrate defect whose repair cut joint tracking error by 46\%.
\end{abstract}
...
$0.952\pm0.002$ ten-second completion against its 0.98 teacher across three seeds
(Fig.~\ref{fig:teaser}c); its prior still consumes the robot-space goal, so we present it
as the actor-side half of the interface program, with the source-side prior specified as
the open step. The same latent then absorbs a second, interaction-rich
...
\item \textbf{A learned command bottleneck for humanoid tracking.} A
  ...
  input---$0.952\pm0.002$ completion against a 0.98 explicit-command teacher (3 training
  seeds, SD), with the reference never visible to the decoder, and a documented failure
  anatomy (path consistency, goal conditioning) for making such bottlenecks work.
...
Table~\ref{tab:main} is the paper's central result. A 64-d co-trained bottleneck as
the actor's only motion input recovers 97\% of its explicit-command teacher's completion
at matched joint RMSE (same DAgger algorithm, same reward stack, three seeds). The frozen
retargeting latent as sole command reaches 0.65 under a PPO recipe; because algorithm,
interaction budget, and seed count differ from the student runs, we report it as context
rather than a controlled contrast---the within-recipe frozen baseline (identical DAgger
pipeline, $z_{\mathrm{ret}}$ command) is running and will replace this row. What Table~I
does establish: the bottleneck costs only 2.8 points against its own teacher, where the
design's failed variants (posterior-path collection, goal-free priors) collapse to
0.001--0.05.
...
On single-clip walking: no, and we mapped the negative space completely. (i)~As sole
command through a co-trained prior, the frozen retargeting latent fails
(0.004--0.016 completion, 3 seeds, identical DAgger recipe). (ii)~Beside the explicit
goal, its additive value straddles zero ($+2.1/+0.5/-0.2$\,pp paired). (iii)~Under
corruption of the robot-space reference---the hypothesized setting where an independent
human-side channel should buy robustness---a 100-rollout screen suggested $+4.0$\,pp at
$\sigma{=}0.5$, but a $10\times$-powered rerun (1{,}024 rollouts per cell, four noise
levels) returned $-1.0$ to $+0.7$\,pp with paired per-seed deltas straddling zero: the
screen's signal was sampling noise, and our pre-specified promotion gate correctly
rejected it. A symmetric control (corrupting the latent channel itself leaves the policy
at its clean performance) confirms the policy learns to ignore the frozen latent. The
one unexplained difference is a $4\times$ tighter seed variance with the latent present
($\pm0.002$ vs $\pm0.008$; $n{=}3$, curiosity not claim).
...
bottleneck-side numbers in this draft (Table~\ref{tab:main}, the additive and
noise-robustness results) are therefore from leak-affected runs and are being
re-measured under the repaired slicing; retargeting-side results, the audit, the
reference-policy baselines, and the trackability matrix are unaffected (their pipelines
never sliced the observation vector). The red flag that should have caught it earlier:
under the leak, arms with \emph{identical} goal information differed by ninety points of
completion---an incoherence input duplication cannot produce.

\section{Limitations}\label{sec:limits}
\textbf{Interface scope --- the central open step.} The current prior consumes the
robot-space goal; the interface-replacement experiment (a prior over
$z_{\mathrm{ret}}$ alone, with tracking gradients optionally reaching the retargeting
encoder) is specified and queued, as are the within-recipe frozen baseline and the
factorial attribution ablation. The latent's additive value over an explicit goal is
null on single-clip walking; multi-clip and cross-embodiment settings are in progress. \textbf{Embodiment generalization.} Zero-shot transfer to
...
results are a reward channel that was silently dead and a set of pre-specified nulls; we
