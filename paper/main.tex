...
0.606, thirty times the goal-free control: the retargeting latent transfers deployable
goal information across the interface. Third, the remaining 36-point gap between
...
iters over 8 heterogeneous clips). We also report a cautionary tale for noise-robustness
claims: a 100-rollout screen suggested the latent buys $+4$\,pp under reference
corruption, and a $10\times$-powered rerun returned zero---though both ran under the
observation-layout defect (\S\ref{sec:defect}), so the robustness question is open, not
answered, and its pre-specified gate discipline (screen, power-up, reject) is the part
that transfers.
