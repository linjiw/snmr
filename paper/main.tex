...
negative-results ledger. On the single-clip cyclic tracking that dominates today's
demonstrations, a trivially constructed clock recovers 79\% of teacher performance
through the same interface: the bar any ``the latent carries motion'' claim must now
clear.
\end{abstract}
...
seed ranges. On a single cyclic clip the deployable content of the retargeting latent
...
retired it. Third, the constructive residue: an exogenous $\sim$64-d clock alone
...
channel to the goal exists, the policy takes it. Together the probes explain the
anatomy's middle rungs: a latent that aligns instances but holds neither semantics nor
contact has little to offer a tracker beyond its time course---and it delivers even
that (\S\ref{sec:exp}) more noisily than clean sinusoids.


\section{Scaling the Interface}
\subsection{Absorbing a second, interaction-rich teacher}
\label{sec:twoteacher}
Yes (Table~\ref{tab:twoteacher}; full root pose predicted in the human heading frame
