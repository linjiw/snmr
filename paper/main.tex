...
CI $[-3.1, +4.9]\%$). This matrix predates the reward repair
(\S\ref{sec:defect}); both sources trained under the identical defect, but a
position-blind assay may under-detect source differences, so a post-repair rerun with a
sensitivity control (deliberately corrupted references) is registered and pending. Formal noninferiority on completion was not established: the
